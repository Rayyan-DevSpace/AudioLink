...
class MainActivity : ComponentActivity() {
    ...
    ) { /* Home screen re-reads device lists once granted; no extra action needed here. */ }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            var spectrumLevels by remember { mutableStateOf(List(24) { 0f }) }
            ...
            LaunchedEffect(screen) {
                if (screen == Screen.INPUT) {
                    spectrumReader.levels().collectLatest { levels -> spectrumLevels = levels }
                }
            }
                ...
                when (screen) {
                    Screen.HOME -> HomeScreen(
                        ...
                        onSplitRoutingToggle = { on ->
                            ...
                            if (on) RoutingForegroundService.start(this@MainActivity)
                            else RoutingForegroundService.stop(this@MainActivity)
                        },
                        onThemeToggle = { dark ->
                            scope.launch {
